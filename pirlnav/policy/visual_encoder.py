...
class VisualEncoder(nn.Module):
    ...
    def set_up_costmap_stem(self):
        ...
        if hasattr(self, 'running_mean_and_var'):
            ...
            new_rmv.to(old_rmv.mean.device)

            # Copy pre-trained ImageNet/PIRLNav stats for the first 3 channels
            new_rmv.mean.data[:, :old_conv1.in_channels, :, :] = old_rmv.mean.data
            new_rmv.var.data[:, :old_conv1.in_channels, :, :] = old_rmv.var.data
            new_rmv.count.data = old_rmv.count.data
